...
const HeapCounter = struct {
    heap: Heap,
    item_counter: u16,
};

pub fn chars_to_heap(allocator: std.mem.Allocator, string: []u8) !HeapCounter {
    var char_map = std.AutoHashMap(u8, i32).init(allocator);

    for (string) |char| {
        const mapChar = char_map.get(char);
        if (mapChar != null) {
            try char_map.put(char, mapChar.? + 1);
        } else {
            try char_map.put(char, 1);
        }
    }

    var heap = try Heap.Heap(TreeNode, lessThanTree).init(allocator);
    var mapIter = char_map.iterator();
    var item_counter: u16 = 0;
    ...
    const heap_counter = try allocator.create(HeapCounter);
    heap_counter.* = .{
        .heap = heap,
...
pub fn build_tree(allocator: std.mem.Allocator, char_heap: *Heap) !*TreeNode {
    var item_counter: usize = 0;

    // At least 2 items in char_heap.
    while (char_heap.pointer > 1) {
        const left = char_heap.remove();
        const right = char_heap.remove();

        // Because we would return the items that are in the array of the char_heap.
        // And the pointers in the array don't change, we would end up with
        // Recursive relations A -> B -> A
        // So to fix this I create copies, seperate.
        // I don't think this is ideal.
        ...
        char_heap.insert(copies[2]);
    }

    return char_heap.remove().?;
}

const BitmaskAndCounter = struct {
    bitmasks: []u64,
    bitmasks_index: usize,
};

pub fn build_bitmask(allocator: std.mem.Allocator, string_content: []u8, tree: *TreeNode) ![]u64 {
    var bitmasks = try allocator.alloc(u64, BUFFER_SIZE);
    var bitmask_index: usize = 0;

    bitmasks[bitmask_index] = 0;
    var bitmask: *u64 = &bitmasks[bitmask_index];

    var bitmask_used: u16 = 0;

    for (string_content) |char| {
        // Pattern starting with 1, so that we don't lose the left 0s
        var pattern = Tree.walk(tree, char, 1).?;
        ...
        if (remaining < bits_used) {
            ...
            // We must relloac, as we reached end of our buffer.
            if (bitmask_index == bitmasks.len) {
                bitmasks = try allocator.realloc(bitmasks, bitmasks.len + BUFFER_SIZE);
            }
    ...
    const bitmasks_counter = try allocator.create(BitmaskAndCounter);
    bitmasks_counter.* = .{
        .bitmasks = bitmasks,
        .bitmasks_index = bitmask_index,
    };

    return bitmasks_counter;
}
